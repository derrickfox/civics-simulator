...
const BUDGET_COLOR = '#065f46';

export default function BudgetProcessMap({ stages, currentStage, completedStages }) {
  return (
    <div className="process-map" aria-label="Federal budget process progress map">
            ...
            <React.Fragment key={stage.id}>
              ...
              >
                <div
                  ...
                  style={
                    status === 'current'
                      ? { background: '#ecfdf5', borderColor: BUDGET_COLOR, boxShadow: `0 0 0 3px rgba(6,95,70,0.2)` }
                      : {}
                  }
                >
                  ...
                  {status === 'current' && (
                    <span className="stage-node__pulse" style={{ borderColor: BUDGET_COLOR }} />
                  )}
                </div>
                <span
                  className="stage-node__label"
                  style={status === 'current' ? { color: BUDGET_COLOR } : {}}
                >
                  {stage.title}
                </span>
              </div>
